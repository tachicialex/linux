...
static void one_bit_adc_dac_set_channel_label(struct iio_dev *indio_dev,
						int num_channels)
{
	struct one_bit_adc_dac_state *st = iio_priv(indio_dev);
	...
	st->labels = devm_kcalloc(indio_dev->dev.parent, num_channels, sizeof(char*), GFP_KERNEL);
	...
	printk(KERN_INFO "one_bit_adc_dac_set_channel_label label no %d\n", num_channels);
	for (i = 0; i < num_channels; i++)
	{
		printk(KERN_INFO "one_bit_adc_dac_set_channel_label label %d: %s\n", i, st->labels[i]);
	}
}
...
static int one_bit_adc_dac_parse_dt(struct iio_dev *indio_dev)
{
	...
	one_bit_adc_dac_set_channel_label(indio_dev, in_num_ch + out_num_ch);
